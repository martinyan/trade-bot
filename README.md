...
- `SEC_13F_BATCH_SYNC_CRON` (optional; 5-field UTC cron expression for the 13F batch rescan)
- `SEC_13F_BATCH_SYNC_RETAIN_REPORT_PERIODS` (defaults to `5`; bounded 13F raw-history retention after the batch rerun)
- `SEC_13F_BATCH_SYNC_LAST_ALLOWED_SLOT` (optional UTC slot in `YYYY-MM-DDTHH:MM`; use this to turn a cron expression into a one-shot rerun)
